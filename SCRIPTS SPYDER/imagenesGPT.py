    ...
    for carpeta_nombre in os.listdir(ruta_imagenes):
        ...
        for imagen_nombre in os.listdir(carpeta_ruta):
            ...
            try:
                ...
                # Verificar que la imagen sea RGB y del tamaño esperado
                if array_imagen.shape != (64, 64, 3):  # (alto, ancho, canales)
                    print(f"Imagen con forma inesperada: {imagen_ruta} -> {array_imagen.shape}")
                    continue

                # Aplanar y normalizar la imagen
                array_imagen = array_imagen.flatten() / 255.0
