...
def upload():
    ...
    for carpeta_nombre in os.listdir(ruta_imagenes):
        ...
        for imagen_nombre in os.listdir(carpeta_ruta):
            ...
            try:
                # Cargar la imagen, convertir a RGB y redimensionarla
                imagen = Image.open(imagen_ruta).convert('RGB').resize(tamaño)
    ...
    try:
        ...
        print("Error al convertir listas en arrays de NumPy:", e)
        return
    ...
    if not os.path.exists(ruta_nueva_imagen):
        print(f"El archivo de prueba '{ruta_nueva_imagen}' no existe. Verifica la ruta y el nombre.")
        return

    try:
        imagen_nueva = Image.open(ruta_nueva_imagen).convert('RGB').resize(tamaño)
